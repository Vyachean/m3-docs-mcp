...
import { normalizeTokenTableSystem } from '../src/json-extraction/render-markdown.js';
      ...
      'designSystems/ds1/tokenSets/ts1/tokens/tok1': {
        contextualReferenceTree: [
          ...
          },
          {
            ...
            referenceTree: {
              tokenName: 'md.comp.button.container.color',
              childNodes: [{ tokenName: 'md.sys.color.primary', childNodes: [{ tokenName: 'md.ref.palette.primary20', childNodes: [] }] }],
            },
          ...
          },
          {
            ...
            referenceTree: {
              tokenName: 'md.comp.button.container.color',
              childNodes: [{ tokenName: 'md.sys.color.primary', childNodes: [{ tokenName: 'md.ref.palette.primary90', childNodes: [] }] }],
            },
...
describe('tokenTableToMarkdown', () => {
  it('generates a table with Name/Token/sys/ref/Light/Dark columns', () => {
    const system = normalizeTokenTableSystem(makeSystem())!;
    const md = tokenTableToMarkdown(system, ['Button - Common']);
  ...
  it('omits HC columns when no high-contrast data exists', () => {
    const raw = makeSystem();
    const trees = raw.contextualReferenceTrees['designSystems/ds1/tokenSets/ts1/tokens/tok1'];
    trees.contextualReferenceTree = trees.contextualReferenceTree.filter(
      (e) => !e.contextTags.includes(HIGH_CONTRAST_TAG)
    );
    const system = normalizeTokenTableSystem(raw)!;
    const md = tokenTableToMarkdown(system, ['Button - Common']);
  ...
  it('returns empty string when no matching token sets', () => {
    const system = normalizeTokenTableSystem(makeSystem())!;
    const md = tokenTableToMarkdown(system, ['NonExistent Set']);
    expect(md).toBe('');
  });

  it('returns empty string when no tokens have tree data', () => {
    const system = normalizeTokenTableSystem(makeSystem({ contextualReferenceTrees: {} }))!;
    const md = tokenTableToMarkdown(system, ['Button - Common']);
    expect(md).toBe('');
  });
});
